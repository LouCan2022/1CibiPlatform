...
/// <summary>
/// The preview decoder must accept what operators actually upload: Excel's ANSI
/// "CSV (Comma delimited)" export (Windows-1252, no BOM) as well as UTF-8 with and
/// without a BOM. Mirrors Test\BackendAPI\BuildingBlocks.UnitTests\CsvTextDecoderTests.cs.
/// </summary>
public class CsvTextDecoderTests
{
	...
	public void Decode_Utf8WithBom_StripsBomFromFirstCell()
	{
		...
		// leaked U+FEFF on "LastName" would break canonical-header matching.
	...
	public void Decode_Windows1252Bytes_DecodesEnyeCorrectly()
	{
		...
		// which a UTF-8 StreamReader turned into U+FFFD.
	...
	{
		// IBrowserFile/OSS streams cannot be rewound; the decoder must buffer once
		// rather than re-read the source for the fallback pass.
		byte[] bytes = [0xD1, 0x75, 0xF1, 0x65, 0x7A];
